...
def integrate_vec(vec, time_dep=False, method='ss', **kwargs):
    ...
    if method in ['ss', 'scaling_and_squaring']:
        ...
        if time_dep:
            ...
            var_delta = 'var_delta' in kwargs and kwargs['var_delta']

            if var_delta:
               
                # if var_delta then vec is [vec, delta] 
                vec, delta = vec

                # phi^(0)
                disp = tf.zeros_like(vec)

                # phi^(1) (scaling)
                vec = vec / (2**nb_steps)

                # determine max necessary step to speed up computation
                max_step = K.max(K.arange(16) * K.cast(delta, tf.int32))

                i = tf.constant(0)

                # loop condition
                cond = lambda i, disp, vec: i <= max_step

                # loop bodies
                # i:    increment loop var
                # disp: add phi^(i+1) (vec) to displacement only if delta[i] == 1
                # vec:  set vec to phi^(i+2) (squaring)
                body = lambda i, disp, vec: [
                        i+1,
                        K.switch(K.equal(delta[i], 1), disp + transform(vec, disp), disp),
                        vec + transform(vec, vec)
                    ]

                _, disp, _ = tf.while_loop(cond, body, [i, disp, vec])
