...
class VecInt(Layer):
    ...
    def __init__(self, indexing='ij', method='ss', int_steps=5, var_delta=True, **kwargs):
        """        
        Parameters:
            method: can be any of the methods in neuron.utils.integrate_vec
            var_delta: if True, layer takes delta as additional input
        """
    ...
    def build(self, input_shape):
        ...
        # if var_delta then input is a list [flow, delta]
        if self.var_delta:
            self.inshape = input_shape[0]
        else:
            self.inshape = input_shape

        if self.inshape[-1] != len(self.inshape) - 2:
            raise Exception('transform ndims %d does not match expected ndims %d' \
                % (self.inshape[-1], len(self.inshape) - 2))

    def call(self, inputs):
        # if var_delta then inputs is a list [flow, delta]
        if self.var_delta:
            loc_shift, delta = inputs
            loc_shift = K.reshape(loc_shift, [-1, *self.inshape[1:]]) # multi-gpu
        ...
        if self.var_delta:
            elems = [loc_shift, delta]
        else:
            elems = loc_shift

        # map transform across batch
        out = tf.map_fn(self._single_int, elems, dtype=tf.float32)
        
        out._keras_shape = loc_shift._keras_shape
    ...
    def _single_int(self, inputs):
        ...
        return integrate_vec(vel, method=self.method,
                      ...
                      time_pt=1, var_delta=self.var_delta)
