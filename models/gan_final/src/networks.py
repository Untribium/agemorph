...
def unet_core(vol_shape, vel_resize, batchnorm, leaky):
    ...
    bn = 'bn' if batchnorm else ''

    e1 = conv_block(e0, 16, 2, batchnorm=batchnorm, leaky=leaky) # 40 16 40  16
    print(K.int_shape(e1), bn)
    e2 = conv_block(e1, 32, 2, batchnorm=batchnorm, leaky=leaky) # 20  8 20  32
    print(K.int_shape(e2), bn)
    e3 = conv_block(e2, 32, 1, batchnorm=batchnorm, leaky=leaky) # 20  8 20  32
    print(K.int_shape(e3), bn)
    e4 = conv_block(e3, 64, 2, batchnorm=batchnorm, leaky=leaky) # 10  4 10  64
    print(K.int_shape(e4), bn)
    e5 = conv_block(e4, 64, 1, batchnorm=batchnorm, leaky=leaky) # 10  4 10  64
    print(K.int_shape(e5), bn)
    e6 = conv_block(e5, 64, 2, batchnorm=batchnorm, leaky=leaky) #  5  2  5  64
    print(K.int_shape(e6), bn)
    
    d6 = conv_block(e6, 64, 1, batchnorm=batchnorm, leaky=leaky) #  5  2  5  64
    print(K.int_shape(d6), bn)
    d5 = upsample_layer()(d6)                                    # 10  4 10  64
    d5 = concatenate([e5, d5])                                   # 10  4 10 128
    d5 = conv_block(d5, 64, 1, batchnorm=batchnorm, leaky=leaky) # 10  4 10  64
    print(K.int_shape(d5), bn)
    d4 = conv_block(d5, 32, 1, batchnorm=batchnorm, leaky=leaky) # 10  4 10  32
    print(K.int_shape(d4), bn)
    d3 = upsample_layer()(d4)                                    # 20  8 20  32
    d3 = concatenate([e3, d3])                                   # 20  8 20  64
    d3 = conv_block(d3, 32, 1, batchnorm=batchnorm, leaky=leaky) # 20  8 20  32
    print(K.int_shape(d3), bn)
    d2 = conv_block(d3, 16, 1, batchnorm=batchnorm, leaky=leaky) # 20  8 20  16
    print(K.int_shape(d2), bn)
    d1 = upsample_layer()(d2)                                    # 40 16 40  16
    d1 = concatenate([e1, d1])                                   # 40 16 40  32
    d1 = conv_block(d1, 16, 1, batchnorm=batchnorm, leaky=leaky) # 40 16 40  16
    print(K.int_shape(d1), bn)
    d1 = conv_block(d1,  8, 1, batchnorm=False,     leaky=leaky) # 40 16 40   8
    print(K.int_shape(d1))

    if vel_resize == 1.0:
        d0 = upsample_layer()(d1)                                # 80 32 80   8
        d0 = concatenate([e0, d0])                               # 80 32 80   9
        d0 = conv_block(d0, 3, 1, batchnorm=False,  leaky=leaky) # 80 32 80   3
...
def generator_net(vol_shape, vel_resize, int_steps, batchnorm, leaky):
    ...
    outputs = [y, flow_params, flow_magnitude, flow]
...
def critic_net(vol_shape, base_nf=8, leaky=0.2, delta_in=False):
    ...
    l = conv_block(l, base_nf*16, 2, leaky=leaky)
    print(K.int_shape(l))
