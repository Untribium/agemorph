...
class GANLosses():
    ...
    def __init__(self, prior_lambda, flow_shape=None):
    ...
    def _degree_matrix(self, vol_shape):
        ...
        # prepare conv kernel
        conv_fn = getattr(tf.nn, 'conv{}d'.format(ndims))
    ...
    def prec_loss(self, y_true, y_pred):
        """
        ...
        y_true: unused, but Keras requires that loss functions have two args
        """
    ...
    def kl_loss(self, y_true, y_pred):
        ...
        if self.flow_shape is None:
            # Note: this might not work in multi_gpu mode if vol_shape is not apriori passed in
            self.flow_shape = y_true.get_shape().as_list()[1:-1]

        # compute the degree matrix (only needs to be done once)
        # we usually can't compute this until we know the ndims, 
        # which is a function of the data
        if self.D is None:
            self.D = self._degree_matrix(self.flow_shape)
        ...
        prec_term = self.prior_lambda * self.prec_loss(y_true, mean)
    ...
    def gradient_penalty_loss(self, y_true, y_pred, y_avg):
        """                                                                    
        compute gradient penalty for generator loss                            
        from github.com/eriklindernoren/Keras-GAN/blob/master/wgan_gp/wgan_gp.py
        """
        grads = K.gradients(y_pred, y_avg)[0] # shape = (batch_size, *vol_size, 1)

        grads_sqr = K.square(grads) 
        grads_sqr_sum = K.sum(grads_sqr, axis=tf.range(1, len(grads_sqr.shape)))
        grads_l2_norm = K.sqrt(grads_sqr_sum)

        grads_penalty = K.square(1 - grads_l2_norm)
        
        return K.mean(grads_penalty)
