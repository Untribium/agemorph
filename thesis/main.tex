...
% Age Regressor
Let $ a_x $ denote a patient's age at the time of taking image $x$ and $ \hat a_x $ denote the age as estimated by the age regressor on $x$. Furthermore, let $ t_{x, y} $ and $ \hat t_{x, y} $ denote the real and estimated time differences between images $x$ and $y$, where $ \hat t_{x, y} = \hat a_y - \hat a_x $

% Dx Classifier
We examine two different loss types


% Similarity Loss (L1)
Similar to \cite{VAGAN} and \cite{wegmayr}, we introduce a similarity loss as the difference between the original image $x$ and the warped image $ x \circ \Phi^{(t)} $ intended to keep the model from producing drastic changes
\begin{equation}
	\mathcal{L}_{sim}( x, \Phi ) = \norm{ (x \circ \Phi^{(t)}) - x }_1
\end{equation}
...
Note that while this loss term acts as a regularizer, the primary motivation for sparseness is to improve the interpretability of the deformation field.
...
\section{Conversion Prediction} \label{sec:convpred}
...
% Conversion
The conversion set consists of image pairs $(x_i, y_i)$ of progressive and stable MCI patients and is used as the test set for our conversion prediction model. The selection criteria largely follow directly from the definitions in \autoref{sec:convpred}. In general, multiple viable image pair combinations exist for each patient.
...
% Leave One Visit Out
To validate the age regressor in a task that more closely matches its role as a loss term in the GAN
